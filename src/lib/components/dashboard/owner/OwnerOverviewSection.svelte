<script lang="ts">
	...
	import DailySalesChart from './DailySalesChart.svelte';
	...
	const totalRevenue = $derived(
		(data.dailySales ?? []).reduce((sum: number, day: { amount: number }) => sum + day.amount, 0)
	);
	const totalBookings = $derived(
		(data.dailySales ?? []).reduce((sum: number, day: { count: number }) => sum + day.count, 0)
	);
	const averageDailyRevenue = $derived(
		(data.dailySales ?? []).length > 0 ? totalRevenue / (data.dailySales?.length ?? 1) : 0
	);
...
<div class="owner-overview-section">
